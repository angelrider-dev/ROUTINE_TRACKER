...
class NotificationService {
  ...
  ///
  /// Returns false (without throwing) if scheduling wasn't possible —
  /// e.g. notification permission isn't granted, or the OS rejected the
  /// exact-alarm request. Callers can surface this to the user instead of
  /// silently losing a reminder.
  Future<bool> rescheduleFor(RoutineTask task) async {
    await cancelFor(task.id);

    if (!task.reminderEnabled || task.scheduledTime == null) return true;

    // Don't attempt to schedule if we don't actually have permission —
    // avoids an uncaught PlatformException on Android 12+ when the
    // exact-alarm permission is missing, and keeps behavior predictable
    // if the user denied notifications outright.
    ...
    try {
      switch (task.recurrence) {
        ...
        case RecurrenceType.oneOff:
          return await _scheduleOnce(task, hour: time.hour, minute: time.minute);
      }
      return true;
    } catch (e) {
      // Scheduling failed (e.g. OS rejected the exact-alarm request on
      // some OEM skins). Swallow rather than crash the app — the task
      // itself is still saved, it just won't remind the user.
      return false;
    }
  }
  ...
  /// Returns false if the one-off's day has already passed, so the
  /// reminder was silently skipped rather than scheduled — callers can
  /// surface this to the user instead of assuming it always succeeds.
  Future<bool> _scheduleOnce(RoutineTask task, {required int hour, required int minute}) async {
    final scheduled = _nextInstanceOfTime(hour, minute, task.createdAt);
    if (scheduled.isBefore(DateTime.now())) return false; // one-off's day already passed
